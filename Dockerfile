...
WORKDIR /aws-ecs-config-files
RUN sudo chown -R nuser:nuser /aws-ecs-config-files/

# Create Gemset
RUN /bin/bash -l -c "rvm gemset create aws-ecs-config-files"
RUN /bin/bash -l -c "rvm use 2.1.2@aws-ecs-config-files --default"

RUN sudo apt-get autoremove -y

# Copy across the rest of the code
ADD . /aws-ecs-config-files
RUN sudo chown -R nuser:nuser .

WORKDIR /aws-ecs-config-files/worker
RUN /bin/bash -l -c "npm install"
WORKDIR /aws-ecs-config-files

VOLUME ["/aws-ecs-config-files"]
